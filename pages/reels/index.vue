<template>
    <section class="reel-page">
        ...
        <Head>
            ...
            <SchemaOrgVideoObject v-if="firstReel" :name="firstReel.title"
                :description="firstReel.title + ' - A cinematic reel by Arafat Sarkar'"
                :thumbnail-url="`${URL}${firstReel.thumbnail}`" :content-url="firstReel.reel_link"
                :upload-date="firstReel.created_at" :publisher="{
                    '@type': 'Person',
                    name: 'Arafat Sarkar',
            ...
            <div class="content-container">
                <div :id="item.category_name" class="inner-content-container" v-for="(item, i) in reelsData" :key="i">
                            ...
                            <div class="reel" v-for="(reel, index) in item.reels" :key="reel.id">
                                ...
                                <div class="thumbnail">
                                    <NuxtImg class="img fit-cover" :src="`${URL}${reel.thumbnail}`"
                                        :alt="`${reel.title} reel thumbnail`" :title="`Watch ${reel.title} reel`"
                    ...
                    <div class="w-100 f-center m-b-30 cur-pointer">
                        <NuxtLink :to="`/reels/${item.category_name}`" :title="`See all reels of ${item.category_name}`"
                                ...
                                <div class="col-1 border">
                                    <div class="w--60">
                                        <NuxtImg class="img square center" :src="`${URL}${item.thumbnail}`"
                                            ...
                                            :title="`Watch ${item.title} related reel`" width="60" height="60"
...
// SEO configuration
const seo = computed(() => ({
    title: 'Arafat Sarkar | Reels Showcase',
    description: 'Explore a curated collection of cinematic reels by Arafat Sarkar, showcasing professional videography and filmmaking in Bangladesh.',
    keywords: 'Arafat Sarkar, reels showcase, cinematography, videography, Bangladesh, filmmaking, portfolio',
    canonical: 'https://arafatsarkar.com/reels',
